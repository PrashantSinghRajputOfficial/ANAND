...
export default function Leadership() {
  ...
  return (
    <div className="bg-white text-slate-900 overflow-hidden">
      <PageBanner 
        title={t('Executive Profile')} 
        subtitle={t('Meet our leadership and explore the strategic vision driving Anand Electricals.')}
      />
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
          ...
          <div className="lg:col-span-4 lg:sticky lg:top-28">
            <div className="bg-slate-50 border border-slate-200/60 rounded-2xl p-6 shadow-lg space-y-6">
              ...
              <div>
                <h2 className="font-heading font-bold text-2xl text-slate-900">{t('Amit Kumar')}</h2>
                <p className="text-industrial-cyan text-xs uppercase tracking-wider font-heading mt-0.5">{t('Founder & Managing Director')}</p>
                <p className="text-slate-600 text-xs mt-3 leading-relaxed">
                  {t('18+ years of hands-on experience in high-voltage industrial engineering, switchgear designs, and grid commissioning.')}
                </p>
              </div>

              <div className="pt-4 border-t border-slate-200 flex items-center justify-between">
                <span className="text-xs text-slate-500">{t('Connect Profile:')}</span>
                <a 
                ...
                >
                  <FaLinkedin size={14} /> {t('LinkedIn Profile')}
                </a>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-8 space-y-12">
            ...
            <div className="space-y-6">
              <span className="font-heading font-semibold text-xs tracking-widest text-industrial-cyan uppercase">
                {t("Director's Message")}
              </span>
              <h3 className="font-heading font-bold text-3xl text-slate-900 tracking-tight">
                {t('Our Mandate: Safe, Continuous Power for Indian Manufacturing')}
              </h3>
              <blockquote className="border-l-3 border-industrial-cyan pl-6 italic text-slate-600 text-base md:text-lg">
                "{t('An industrial website should convey competence, not flash. At Anand Electricals, we build critical systems that factories rely on for continuous operations. Our goal is to offer B2B developers engineering stability, transparency, and top-tier safety certifications.')}"
              </blockquote>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-200">
              <div>
                <h4 className="font-heading font-semibold text-slate-900 mb-2">{t('Operational Vision')}</h4>
                <p className="text-slate-600 text-xs leading-relaxed">
                  {t('Establish Anand as the standard of choice for industrial control grids, maintaining high-conductivity thresholds and 100% FAT test transparency.')}
                </p>
              </div>
              <div>
                <h4 className="font-heading font-semibold text-slate-900 mb-2">{t('Contracting Mission')}</h4>
                <p className="text-slate-600 text-xs leading-relaxed">
                  {t('Provide developers with pre-fabricated, modular Changeover AMF and MCC panels that reduce commissioning lags by up to 25%.')}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-6 pt-6 border-t border-slate-200">
              <h3 className="font-heading font-bold text-2xl text-slate-900 flex items-center gap-3">
                <FaAward className="text-industrial-cyan" /> {t('Achievements & Credentials')}
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-5 bg-slate-50 border border-slate-200/60 rounded-lg flex items-start gap-4 shadow-sm">
                  ...
                  <div>
                    <h4 className="font-heading font-semibold text-slate-900 text-sm">{t('515+ Projects Completed')}</h4>
                    <p className="text-slate-600 text-xs mt-1">{t('Successfully deployed industrial panel infrastructure projects across Rajasthan and beyond.')}</p>
                  </div>
                </div>
                <div className="p-5 bg-slate-50 border border-slate-200/60 rounded-lg flex items-start gap-4 shadow-sm">
                  <div className="w-10 h-10 rounded bg-industrial-cyan/5 border border-industrial-cyan/20 flex items-center justify-center text-industrial-cyan font-bold flex-shrink-0 text-sm">
                  ...
                  <div>
                    <h4 className="font-heading font-semibold text-slate-900 text-sm">{t('Grade-A Contracting License')}</h4>
                    <p className="text-slate-600 text-xs mt-1">{t('Authorized by the State Electrical Inspectorate to execute massive utility line jobs.')}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6 pt-6 border-t border-slate-200">
              <h3 className="font-heading font-bold text-2xl text-slate-900 flex items-center gap-3">
                <FaRoad className="text-industrial-cyan" /> {t('Timeline of Growth')}
              </h3>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 justify-center">
            ...
            <div className="bg-white border border-slate-200/60 rounded-2xl p-6 shadow-md space-y-6 max-w-sm mx-auto w-full">
              ...
              <div>
                <h3 className="font-heading font-bold text-xl text-slate-900">{t('Deepesh Singh Rajput')}</h3>
                <p className="text-industrial-cyan text-xs uppercase tracking-wider font-heading mt-0.5">{t('Online Team Manager')}</p>
                <p className="text-slate-600 text-xs mt-3 leading-relaxed">
                  {t('Coordinates digital workflows, online client queries, site proposals, and online sales operations.')}
                </p>
              </div>

              <div className="pt-4 border-t border-slate-200 flex items-center justify-between">
                <span className="text-xs text-slate-500">{t('Connect:')}</span>
                <a 
                  href="https://linkedin.com" 
                  target="_blank" 
                  rel="noopener noreferrer" 
